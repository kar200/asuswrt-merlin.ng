...
static int env_dev_read_spinor( int addr, size_t * len, char * buffer )
{
	...
	ret = mtd_read(mtd,addr,*len,&retlen,(unsigned char*)buffer);
	put_mtd_device(mtd);
	return ret;
}
...
#endif
#ifdef CONFIG_NAND
static struct mtd_info *get_nand_env_dev(void)
{
	int bdev;
...
static int env_dev_write_nand( int addr, size_t * len, char * buffer )
{
	...
	nand_erase(mtd, (loff_t)blocknum * blocksize, blocksize);
	ret = nand_write(mtd, (loff_t)blocknum * blocksize, &blocksize,
			       (void *)block);
	if (ret < 0) {
