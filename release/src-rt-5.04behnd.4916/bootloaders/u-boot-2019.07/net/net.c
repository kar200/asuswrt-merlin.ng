...
/*
 * Broadcom recovery hook.  The board's built-in HTTP recovery server
 * (board/broadcom/bcmbca/httpd/bcmbca_net.c) installs http_rcv() here so
 * it can tap incoming UDP packets.  This global used to live in the
 * Broadcom-modified net.c which is only shipped precompiled for 32-bit
 * ARM in this SDK; define it here so the aarch64 build links.
 */
