...
/*
 * Ported from the matching kernel MDIO driver
 * (bcmdrivers/opensource/phy/mdio_drv_common.c).  The U-Boot copy of this
 * file in this SDK was missing these two exports, which mdio_drv_v1.c
 * needs.
 */
...
int32_t mdio_cfg_set(uint32_t *p, uint16_t probe_mode, uint16_t mdio_clock_divider)
{
    ...
    WRITE_32(p, cfg);
    printf("MDIO Clock Divider Value: %d Probe mode: %s\n",
