...
static int _phy_cfg(uint32_t port_map)
{
    ...
    if (!port_map)
    {
        /* Sercomm HG6244B has 4 internal EGPHY ports (1..4) wired to LAN1..LAN4 */
        port_map = 0xf;
    }
...
static int _phy_afe_all(void)
{
    ...
    phy_dev.bus_drv = bus_drv_get(BUS_TYPE_MDIO_V1_INT);
...
static int _phy_leds_init_51XX(phy_dev_t *phy_dev, void *_leds_info, uint8_t is_shifted)
{
    ...
    if (!leds_info || (!leds_info->link[0] && !leds_info->link[1]))
    {
        /* Sercomm HG6244B CFE default LED configuration */
        led_shd1c_02 = 0x206;
        led_shd1c_09 = 0;
        led_shd1c_0d = 0x00a0;
        led_shd1c_0e = 0x000a;
        led_core_exp_04 = 0x0128;
    }
    else if (((leds_info->link[0] == LED_SPEED_GBE || leds_info->link[0] == LED_SPEED_ALL) &&
    ...
    /*
     * CFE-only EGPHY programming, observed on the wire from the stock CFE
     * (HANDOVER §74.4).  The vendor stack issues these two indirect (RDB)
     * writes during MAC/PHY init, immediately after the LED configuration
     * above, and this driver never did:
     *
     *     0xa420 data is 0x23.   ->  RDB_ACCESS | 0x2420 <- 0x23
     *     0xb800 data is 0xc0.   ->  RDB_ACCESS | 0x3800 <- 0xc0
     *     0xb800 data is 0x80.   ->  RDB_ACCESS | 0x3800 <- 0x80
     *
     * Nothing in this SDK references 0x2420 or 0x3800.  They are written
     * unconditionally here; the values are identical for every EGPHY, so this
     * is safe whether the registers are per-PHY or global.  This is a
     * hypothesis for the missing copper link (§73) -- remove it if it does
     * not help.
     */
    ret = ret ? ret : phy_dev_write(phy_dev, RDB_ACCESS | 0x2420, 0x23);
    ret = ret ? ret : phy_dev_write(phy_dev, RDB_ACCESS | 0x3800, 0xc0);
    ret = ret ? ret : phy_dev_write(phy_dev, RDB_ACCESS | 0x3800, 0x80);
    printk("EGPHY CFE-only RDB writes: 0x2420=0x23 0x3800=0xc0/0x80\n");
