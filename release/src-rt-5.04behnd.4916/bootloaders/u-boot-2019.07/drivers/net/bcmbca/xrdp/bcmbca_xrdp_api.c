...
static void fix_bbh_rx_index(struct cpu_rx_params *rx_params)
{
#if defined(CONFIG_BCM96858)
    /* flowid swaps rx bbh; until we re-generate datapath, swap it here.. */
	if (rx_params->src_bridge_port == 0)
        rx_params->src_bridge_port = 4;
    else if (rx_params->src_bridge_port == 4)
        rx_params->src_bridge_port = 0;
#endif
}
...
static uint8_t bbh_ingress_counter[16];
...
#if defined(CONFIG_BCM6888) || defined(CONFIG_BCM68880) 
     	uint32_t tx_bb_id_map[] = {BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_LAN_1, BB_ID_TX_LAN, BB_ID_TX_LAN, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1
		, BB_ID_TX_LAN_2, BB_ID_TX_LAN, BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_PON_ETH_PD};

        uint32_t tx_port_index_map[] = {0, 1, 2, 3, 0, 0, 1, 1, 2, 3, 4, 5, 4, 2, 5, 6, 0};
#elif defined(CONFIG_BCM6837)
     	uint32_t tx_bb_id_map[] = { BB_ID_TX_LAN,   BB_ID_TX_LAN,   BB_ID_TX_LAN,   BB_ID_TX_LAN, BB_ID_TX_LAN, 
		 							BB_ID_TX_LAN_1, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1, BB_ID_TX_LAN_1,
									BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, BB_ID_TX_LAN_2, 
									BB_ID_TX_PON_ETH_PD};

        uint32_t tx_port_index_map[] = {0, 1, 2, 3, 4, 0, 1, 2, 3, 0, 1, 2, 3, 0};
#endif

static inline uint32_t __rdd_cpu_tx_get_bb_id(uint8_t tx_port)
{
#if defined(CONFIG_BCM6858)
	uint32_t tx_map[] = {BB_ID_TX_BBH_0, BB_ID_TX_BBH_1, BB_ID_TX_BBH_2,
		BB_ID_TX_BBH_3, BB_ID_TX_BBH_4, BB_ID_TX_BBH_5, BB_ID_TX_BBH_6};
...
#elif defined(CONFIG_BCM6888) || defined(CONFIG_BCM6837) || defined(CONFIG_BCM68880)
        return (tx_bb_id_map[tx_port] + (tx_port_index_map[tx_port] << 6));
#else
	return (BB_ID_TX_LAN + (tx_port << 6));
#endif
}

static void *get_bbh_tx_counter_id(uint8_t tx_port)
{
	void *counter_id;

#if defined(CONFIG_BCM6888) || defined(CONFIG_BCM6837) || defined(CONFIG_BCM68880)
		switch (tx_bb_id_map[tx_port])
		{
			case BB_ID_TX_LAN:
				counters_table =(uint32_t *)RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE_PTR(cfe_core_runner_image);
			break;

			case BB_ID_TX_LAN_1:
				counters_table =(uint32_t *)RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE1_PTR(cfe_core_runner_image);
			break;

			case BB_ID_TX_LAN_2:
				counters_table =(uint32_t *)RDD_DS_TM_BBH_TX_EGRESS_COUNTER_TABLE2_PTR(cfe_core_runner_image);
			break;
        ...
        counter_id +=  tx_port_index_map[tx_port] * EGRESS_COUNTER_SIZE;
		/*printf("counter index=%d\n", tx_port_index_map[tx_port]);*/
#else
...
#endif
	{
		...
		counter_id = counters_table;
#if defined(CONFIG_BCM6858)
		/* BBH counter is not swapped - so swap index to point to correct port counter */
		if (tx_port == 0) 
			counter_id += 4 * EGRESS_COUNTER_SIZE;
		else if (tx_port == 4) 
			counter_id += 0 * EGRESS_COUNTER_SIZE;
		else
#endif
		/* egress counters are either 8 or 64 bit alignment */
