    ...
    struct ktermios *termios, struct ktermios *old)
{
    ...
    /* Wait until TXFIFO is empty before changing settings (bounded to avoid hang) */
    {
        int txf_limit = 5000;
        while (!((UART_REG(port))->intStatus & TXFIFOEMT) && --txf_limit)
            udelay(5);
    }
     ...
     * If bootloader already configured 115200 baudword, keep existing working value.
     */
    ...
    if ((UART_REG(port))->baudword == 0 || baud != 115200)
...
{
    int wait_limit = 1000;
    while (((UART_REG(port))->txf_levl > BCMTXFIFODRAINTHRESH + 1) && --wait_limit) 
    {
       udelay(5); 
    }
