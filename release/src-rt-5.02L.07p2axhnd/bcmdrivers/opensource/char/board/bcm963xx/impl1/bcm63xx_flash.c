...
void __init kerSysEarlyFlashInit( void )
{
    ...
    if ((BpSetBoardId(inMemNvramData.szBoardId) != BP_SUCCESS)) {
        printk("\n*** Board is not initialized properly from szBoardId '%s', falling back to '968360BG' ***\n\n",
               inMemNvramData.szBoardId);
        strncpy(inMemNvramData.szBoardId, "968360BG", sizeof(inMemNvramData.szBoardId) - 1);
        inMemNvramData.szBoardId[sizeof(inMemNvramData.szBoardId) - 1] = '\0';
        if (BpSetBoardId("968360BG") != BP_SUCCESS)
            printk("\n*** Failed fallback to board ID 968360BG! ***\n\n");
        else
            printk("Successfully initialized fallback Board ID: 968360BG\n");
    }
#if defined(CONFIG_BCM_VOICE_SUPPORT)
    if ((BpSetVoiceBoardId(inMemNvramData.szVoiceBoardId) != BP_SUCCESS)) {
        printk("\n*** Voice Board id is not initialized properly from '%s', falling back to 'LE9652_ZSI_IB' / '968360BG' ***\n\n",
               inMemNvramData.szVoiceBoardId);
        strncpy(inMemNvramData.szVoiceBoardId, "LE9652_ZSI_IB", sizeof(inMemNvramData.szVoiceBoardId) - 1);
        inMemNvramData.szVoiceBoardId[sizeof(inMemNvramData.szVoiceBoardId) - 1] = '\0';
        if (BpSetVoiceBoardId("LE9652_ZSI_IB") != BP_SUCCESS && BpSetVoiceBoardId("968360BG") != BP_SUCCESS)
            printk("\n*** Failed fallback to Voice Board ID! ***\n\n");
        else
            printk("Successfully initialized fallback Voice Board ID\n");
    }
...
int crashFileSet(const char* filename)
{
	...
	printk("ret %d retlen %zu buff %c %c %c\n",
